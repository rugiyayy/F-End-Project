...
let flightRrow = document.querySelector(".flight-row");
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  searchForm.elements["departure"].value = depInp;
  searchForm.elements["arrival"].value = arrivalInp;
...
async function Flights() {
  ...
  let responce = await fetch(`${myUrl}/flights?from=TR&to=GE`);
  let data = await responce.json();

  console.log(data);

  if (!responce.ok) {
    throw new Error("There is no flight");
  }

  if (data) {
    flightRrow.innerHTML = "";
    ...
    flightRrow.innerHTML += `
          ...
          <div class="flight-details">
            <div class="deparutre-details">
              <span class="dep-time">${data.OTA_AirDetailsRS.FlightDetails[0].$.FLSDepartureDateTime}</span>
              <p class="dep-country">${data.OTA_AirDetailsRS.FlightDetails[0].$.FLSDepartureName}</p>
            </div>
          </div>
          ...
          <div class="flight-duration">
            ...
            <div class="duration">
              <p><span>33H 10M,</span><span> 1-stop</span></p>
            </div>
          </div>
      
          <div class="arrival-details">
            <span class="arrival-time">4:45 PM</span>
            <p class="arrival-country">LAX</p>
          </div>
        </div>
  ...
  //     .then((x) => x.json())
  //     .then((data) => {
  //     //   let fromCode = data.Airports.Airport.find((x) => x.$.Country === from).$
  //     //     .IATACode;
  //     //   let toCode = data.Airports.Airport.find((x) => x.$.Country === to).$
  //     //     .IATACode;
